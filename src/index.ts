type WeaponPartCategory = "Trigger Assembly/Fire Control Group" | "Front Sight" | "Rear Sight" | "Scope" | "Front Grip Rail" | "Handguard" | "Barrel" | "Muzzle" | "Dust Cover" | "Magazine" | "Weapon" | "Bolt Carrier Group" | "Dust Cover Rail" | "Tactical Rail" | "Muzzle Device" | "Stock" | "Grip" | "Frame" | "Charging Handle"  | "Internal"| "Forward Assist" | "Upper Receiver" | "Lower Receiver";
...
class Weapon implements Item {
    ...
    public add_attachment(att: AttachmentNode): Weapon {
        ...
        const compatibleSlots = this.components.filter(component => 
    ...
    public remove_attachment(att: AttachmentNode): Weapon {
        ...
        if (parent.removeChild(componentToRemove)) {
            ...
            const getAllChildren = (node: AttachmentNode): AttachmentNode[] => {
                return [node, ...node.children.flatMap(child => getAllChildren(child))];
            };

            const removedNodes = getAllChildren(componentToRemove);
            removedNodes.forEach(node => this.amplifier(node, false))
            this.components = this.components.filter(c => !removedNodes.includes(c as AttachmentNode));
    ...
    private amplifier(att: AttachmentNode, isAdding: boolean): void {
        ...
        Object.keys(stats).forEach((key) => {
            this.initialStats[key as keyof WeaponStats] += stats[key as keyof WeaponStats] * multiplier;
    });
...
class AttachmentNode implements Item {
    ...
    public listChildrenXML(): string {
        const getChildrenNames = (node: AttachmentNode, depth: number = 0): string => {
            ...
            const childNames = node.children.map(child => {
                ...
                const closingTag = childrenStr ? `\n${indent}</attachment>` : `</attachment>`;
                return `\n${childStr}${childrenStr}${closingTag}`;
            });
            return childNames.join('');
        }
        return `<attachments>\n    <attachment name="${this.name}" type="${this.specs.type}">${getChildrenNames(this, 2)}\n    </attachment>\n</attachments>`;
    }

    public canAttach(child: AttachmentNode): boolean {
        return child.specs.type in this.compatibleAttachments 
    ...
    public addChild(child: AttachmentNode): boolean {
        if ((this.compatibleAttachments[child.specs.type] ?? 0)<= 0) {
            console.log(`${child.name} cannot be attached to ${this.name} because there are no compatible slots`);
            return false;
        }

        child.parent = this;
        this.children = [...this.children, child];
        this.compatibleAttachments[child.specs.type] = (this.compatibleAttachments[child.specs.type] ?? 0) - 1
    ...
    public findAttachment(predicate: (node: AttachmentNode) => boolean): AttachmentNode | null {
    ...
    public findAllAttachments(predicate: (node: AttachmentNode) => boolean): AttachmentNode[] {
...
abstract class WeaponFactory {
    ...
    protected createComponent(
        ...
        return new AttachmentNode(name, modelPath, specs, compatibleAttachments, parent, stats);
    }
    ...
    public createWeapon(): Weapon {
        ...
        const weapon = new Weapon(
            upperReceiver,
            this.weaponName,
            { type: "Weapon", description: this.weaponDescription }
        );
        ...
        weapon.components = this.getAllComponents(upperReceiver);
        return weapon;
    }

    private getAllComponents(node: AttachmentNode): AttachmentNode[] {
        return [node, ...node.children.flatMap(child => this.getAllComponents(child))];
    }
}
...
class AR15Factory extends WeaponFactory {
    ...
    protected createUpperReceiver(): AttachmentNode {
        ...
        return this.createComponent(
            ...
            { damage: 0, accuracy: 2, verticalRecoil: 15, horizontalRecoil: 18, ergonomics: -2 }
        );
    }
    ...
    protected createLowerReceiver(): AttachmentNode {
        ...
        return this.createComponent(
            ...
            { damage: 0, accuracy: 0, verticalRecoil: 0, horizontalRecoil: 0, ergonomics: 0 }
        );
    }
    ...
    protected createBarrel(): AttachmentNode {
        ...
        return this.createComponent(
            ...
            { damage: 50, accuracy: 15, verticalRecoil: 10, horizontalRecoil: 8, ergonomics: -5 }
        );
    }
    ...
    protected createInternalParts(): AttachmentNode[] {
        ...
        const bcg = this.createComponent(
            ...
            "Standard BCG",
            {},
            { damage: 0, accuracy: 2, verticalRecoil: 5, horizontalRecoil: 3, ergonomics: 0 }
        );
        ...
        const chargingHandle = this.createComponent(
            ...
            {},
            { damage: 0, accuracy: 0, verticalRecoil: 0, horizontalRecoil: 0, ergonomics: 1 }
        );
    ...
    protected createExternalParts(): AttachmentNode[] {
        ...
        const handguard = this.createComponent(
            ...
            { damage: 0, accuracy: 2, verticalRecoil: -2, horizontalRecoil: -2, ergonomics: -2 }
        );
